...
class main_eventController extends Controller
{
    ...
    public function main_event_datatable()
    {
      if (Auth::user()->m_role == 'admin') {
      	$data = $this->model->d_event()->with('m_category_event')->get();
      }else{
        $data = $this->model->d_event()->with('m_category_event')->where('e_created_by',Auth::user()->m_id)->get();
      }
	    $data = collect($data);
    ...
    {
      // dd($req->all());
      $get_data             = $this->model->d_event()->where('e_id',$req->id)->first();
      Storage::delete($get_data->e_poster);
      $delete             = $this->model->d_event()->where('e_id',$req->id)->delete();
